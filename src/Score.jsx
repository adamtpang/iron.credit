...
function scoreFrom({ years, chains, repaid, value }) {
  let s = 500;
  s += Math.min(years * 30, 150); // up to +150 for years holding
  s += Math.min(chains * 15, 75); // up to +75 for assets held
  s += repaid ? 120 : 0; // proven repayment is the strongest signal
  s += value >= 100000 ? 100 : value >= 10000 ? 60 : value >= 1000 ? 25 : 0;
...
export default function Score() {
  ...
  const score = scoreFrom({ years, chains, repaid, value: Number(value) || 0 });
  const grade = GRADES.find((x) => score >= x.min);
  const limit = (Number(value) || 0) * grade.ltv;
  ...
  const inputStyle = { width: "100%", padding: "11px 13px", borderRadius: 11, border: `1px solid ${C.line}`, background: C.panel, color: C.ink, fontSize: 14, outline: "none" };
      ...
      <article style={{ maxWidth: 920, margin: "0 auto", padding: "16px 20px 60px" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p className="body" style={{ fontSize: 16, lineHeight: 1.55, color: C.mut, maxWidth: 560, margin: 0 }}>
            No credit check, no income proof. Your crypto is your qualification. Your collateral sets the limit, your on-chain reputation sets the rate. Free, instant, and we never see your keys.
          </p>
        </div>

        <div className="score-2" style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16 }}>
          {/* Inputs */}
          <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, padding: 20, display: "grid", gap: 16, alignContent: "start" }}>
            <Field label="Crypto you would lock as collateral" hint="USD value">
          ...
          <div style={{ background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 18, padding: 20 }}>
            ...
            <div className="flex items-center" style={{ gap: 16, marginBottom: 18 }}>
              <div className="disp" style={{ fontSize: 56, fontWeight: 800, color: toneColor, lineHeight: 1 }}>{grade.g}</div>
              <div>
                <div className="disp tnum" style={{ fontSize: 28, fontWeight: 800, color: C.ink, lineHeight: 1 }}>{score}</div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {[
                ...
                ["Interest rate", `~${grade.apr}% APR`, "versus 20 to 30% on an unsecured card"],
                ["Credit check", "None", "your collateral qualifies you"],
              ].map(([k, v, note], i) => (
                <div key={i} className="flex items-center justify-between" style={{ padding: "10px 0", borderTop: i ? `1px solid ${C.line}` : "none" }}>
        ...
        {/* Grade rubric, transparent by default */}
        <section style={{ marginTop: 22 }}>
          ...
          <p className="body" style={{ fontSize: 12, color: C.mut, margin: "10px 2px 0", lineHeight: 1.5 }}>
            Illustrative and educational, not a credit offer. Real terms require identity verification and a live on-chain assessment. As you borrow and repay with Iron, your grade rises, which raises your LTV and lowers your rate, the path toward unsecured credit.
          </p>
        </section>
        ...
        <section id="waitlist" style={{ marginTop: 22, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, padding: 22 }}>
          ...
          <p className="body" style={{ fontSize: 14.5, lineHeight: 1.55, color: C.mut, margin: "0 0 14px", maxWidth: 520 }}>
            Join the waitlist and we will tell you the moment your grade can be turned into a real Iron card and credit line.
          </p>
